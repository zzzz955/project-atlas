...
// 🔴 cpp-style.md §4.1 - the width is the caller's explicit choice (UInt16 for a port, UInt32 for
// the ids and worker counts), so a value that does not fit is a conversion failure rather than a
// silent wrap. std::from_chars reports that as errc::result_out_of_range.
...
ServerConfig ServerConfig::FromIni(const IniDocument& document) {
    ...
    // §14 - read and stored, never branched on.
    ...
    config.log.retention_days = RequireIniUnsigned<UInt32>(document, kLogSection, "retention_days");
