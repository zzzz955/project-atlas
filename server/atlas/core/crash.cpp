// =============================================================================
// [AD 11.1] 크래시 덤프 설치. Windows 는 미니덤프 전용 스레드, Linux 는 커널 코어
// =============================================================================
...
#if defined( _WIN32 )
// DbgHelp 는 Windows 타입 선언을 스스로 포함하지 않음. 알파벳 순 대신 의존 순서
// clang-format off
#include <Windows.h>
#include <DbgHelp.h>
// clang-format on
...
[[noreturn]] void TerminateHandler() noexcept
{
    ...
    // 시그널 핸들러가 아닌 정상 실행 문맥. 덤프 생성 전에 로그 꼬리를 남김
    LogFlush();

#if defined( _WIN32 )
    // MiniDumpWriteDump 는 최상위 필터의 EXCEPTION_POINTERS 로 오류 지점을 짚음
...
std::filesystem::path WindowsDumpPath()
{
    ...
    std::swprintf( filename.data(), filename.size(), L"%ls-%04u%02u%02uT%02u%02u%02uZ-p%lu-%hs.dmp",
                   ...
                   now.wMinute, now.wSecond, static_cast< unsigned long >( process_id ),
                   ATLAS_BUILD_ID );
...
DWORD WINAPI DumpThread( void* )
{
    for ( ;; )
    {
        ...
        if ( file != INVALID_HANDLE_VALUE )
        {
            ...
            // [AD 15.5h] MINIDUMP_TYPE 은 플래그 enum. 분석기가 구분하지 못함
            // 마커는 한 줄만 덮으므로 반드시 코드 줄에 붙일 것
            ...
            const BOOL dump_written =
                ::MiniDumpWriteDump( ::GetCurrentProcess(), ::GetCurrentProcessId(), file,
                                     kDumpType, &exception_info, nullptr, nullptr );
            std::ignore = ::CloseHandle( file );
            if ( dump_written == FALSE )
            {
                // 부분 파일은 유효한 사고 산출물처럼 보이지만 열리지 않음
...
LONG WINAPI UnhandledExceptionFilter( EXCEPTION_POINTERS* exception )
{
    if ( ::InterlockedCompareExchange( &g_windows_crash.dump_started, 1, 0 ) == 0 )
    {
        ...
        // 망가진 덤프 경로가 이미 실패한 프로세스를 영원히 붙들면 안 됨
        std::ignore = ::WaitForSingleObject( g_windows_crash.completed_event, 10'000 );
    }
    else
    {
        // 두 번째 크래시 스레드가 첫 덤프 작성 중 프로세스를 끝내면 안 됨
        std::ignore = ::WaitForSingleObject( g_windows_crash.completed_event, 10'000 );
    }
...
{
    // async-signal-safe 호출만 허용. 심볼화 - 할당 - spdlog - filesystem 은 의도적 배제
    // 커널 코어가 크래시 스레드를 보존하고 오프라인 심볼라이저가 붙임
    ...
    // 전달된 시그널은 핸들러 동안 블록됨
    // 재전송 전에 풀지 않으면 kill 이 큐에 쌓이기만 한다
    // 그러면 아래 _exit 가 커널 코어 경로를 건너뛴다
...
void InitPlatformCrashDiagnostics( const CrashDiagnosticsConfig& )
{
    ...
    for ( std::size_t index = 0; index < kFatalSignals.size(); ++index )
    {
        ...
        if ( ::sigaction( kFatalSignals[index], &action, &g_previous_actions[index] ) != 0 )
        {
            ...
            {
                std::ignore = ::sigaction( kFatalSignals[installed], &g_previous_actions[installed],
                                           nullptr );
            }
...
void CrashDiagnosticsInit( const CrashDiagnosticsConfig& config )
{
    ...
    // DbgEng 최초 초기화가 김
    // 프로세스 시작 때 미리 지불해 예외 경로가 I/O 워커를 멈추지 않게 한다
    std::ignore = CaptureStackTrace( 1 );
    g_previous_terminate = std::set_terminate( &TerminateHandler );
    g_initialized = true;
    ATLAS_LOG_INFO( "crash diagnostics ready [build={} directory={}]", BuildId(),
                    config.directory );
}
