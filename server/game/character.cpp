...
// 🔴 The names carry the table. cpp-style.md §2.2: with unity build ON several sources become one
// translation unit, so two files' anonymous-namespace helpers named `ReadUnsigned` collide — and
// the collision only appears in the unity configuration, which is the one the CI gate runs second.
...
Character::Character(atlas::UInt16 server_id, atlas::CharacterId character_id,
    ...
    // 🔴 The world identity is derived from the durable one rather than allocated separately. A
    // second counter would mean the same character answers to two ids depending on which server
    // asked, and reconciling them is a bug source with nothing on the other side of the trade.
    : Entity(static_cast<atlas::ActorId>(atlas::IdValue(character_id)), pos_x, pos_y, level),
...
atlas::generated::CharactersRow CharactersRowFromDb(const atlas::DbRow& row) {
    ...
    mapped.pos_x_ =
        static_cast<atlas::Int32>(ReadCharacterSigned(row, atlas::generated::kCharactersColPosX));
    mapped.pos_y_ =
        static_cast<atlas::Int32>(ReadCharacterSigned(row, atlas::generated::kCharactersColPosY));
    ...
    // The one nullable column in the table: SQL NULL arrives as std::monostate (§10.3), and the
    // mapper is where that becomes an empty optional instead of a zero timestamp.
    ...
    if (std::holds_alternative<std::monostate>(last_login)) {
        ...
        mapped.last_login_at_ = ReadCharacterTime(row, atlas::generated::kCharactersColLastLoginAt);
    }
    return mapped;
}

std::vector<atlas::DbValue> CharactersUpdateParameters(const atlas::generated::CharactersRow& row) {
    const std::array<DbValue, atlas::generated::kCharactersColumnCount> by_column{
        ...
        row.last_login_at_.has_value() ? DbValue{*row.last_login_at_} : DbValue{std::monostate{}}};
