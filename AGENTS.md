...
| `docs/conventions/cpp-style.md` | **Coding convention SoT.** Namespace, naming, type rules (fixed-width ints, A-plan aliases, strong-typed IDs), macro policy, template policy, the 3-layer mechanical enforcement, layout (`§7.1` — Allman, spaces in parens/angles), and **comments `§9`** (Korean, noun-final, ASCII only, 80-col banners, `[AD x.y]` refs) | — |
...
(`server/vcpkg.json`, 8 deps, `builtin-baseline` pinned by `setup.bat`) + PCH + unity build.
...
emitted `.h`/`.cpp` through the repo `.clang-format` themselves (`tools/cxx-format.js`), so the old
"generated output is never formatted" carve-out no longer holds — and two files under `generated/`
were hand-written all along (`generated/{db,pkt}/tests/*_test.cpp`). `clang-tidy` still excludes it.
...
| `tools/` | **Node data-pipeline = the templatization seam** (`AD §14`). `config-loader.js` + `types.json` (normalized-type SoT, `cpp` column = the C++ seam) + `all_generator.bat`, and **all three** generators `{info,pkt,db}_generator/`. `cxx-format.js` is the shared write-side hook: each generator's `writeTextFile` pipes emitted `.h`/`.cpp` through the repo `.clang-format`, so generated code tracks a style change for free. It requires **clang-format >= 19** and fails loudly below that (`SpacesInParens` is 17+ syntax; 18.1.3 measured `ColumnLimit` in bytes, which moves every Korean-comment wrap — `AD §15.5f`). Driven from the repo root by `npm run gen:*`. See `tools/AGENTS.md`.<br>`tools/loadreport/` is **not** a generator — it renders a load run's JSONL into a self-contained HTML (`npm run loadreport`, `AD §16.1a`). It is deliberately outside `gen:all`/`gen:check`: its input is a measurement, not a source, so drift against it is meaningless |
